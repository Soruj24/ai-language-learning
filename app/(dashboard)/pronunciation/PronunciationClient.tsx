...
import { Mic, Square, Play, RefreshCw, Volume2, Globe, AlertCircle, Lock } from 'lucide-react';
...
const PRACTICE_WORDS = [
  "schedule", "water", "aluminum", "advertisement", "tomato", "vase", "lever", "vitamin", "privacy", "mobile"
];
...
export default function PronunciationClient({ isPro }: PronunciationClientProps) {
  const { t, learningLanguage } = useLanguage();
  const [selectedWord, setSelectedWord] = useState(PRACTICE_WORDS[0]);
  const [accent, setAccent] = useState<'US' | 'UK'>('US');
  const [refAudioBuffer, setRefAudioBuffer] = useState<AudioBuffer | null>(null);
      ...
      <div className="flex items-center justify-center h-[60vh]">
        <Card className="w-full max-w-md text-center border-primary shadow-lg">
          <CardHeader>
            ...
            <CardDescription>
              Pronunciation Analysis is available for Pro and Premium members only.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <p className="text-muted-foreground mb-4">
              Get real-time feedback on your accent and improve your speaking skills with advanced AI analysis.
            </p>
  ...
  const fetchReferenceAudio = async () => {
    ...
    try {
      ...
      if (!Array.isArray(data) || data.length === 0) throw new Error("Word not found");

      const phonetics = data[0].phonetics;
      // Find audio based on accent
      let audioUrl = '';
      if (accent === 'US') {
        audioUrl = phonetics.find((p: any) => p.audio?.endsWith('-us.mp3'))?.audio || 
                   phonetics.find((p: any) => p.audio?.includes('us'))?.audio;
      } else {
        audioUrl = phonetics.find((p: any) => p.audio?.endsWith('-uk.mp3'))?.audio || 
                   phonetics.find((p: any) => p.audio?.includes('uk'))?.audio;
      }

      // Fallback to any audio if specific accent not found
      if (!audioUrl) audioUrl = phonetics.find((p: any) => p.audio)?.audio;
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h2 className="text-3xl font-bold tracking-tight">Pronunciation Lab</h2>
          <p className="text-muted-foreground">Master your accent with AI-powered analysis</p>
        </div>
        <div className="flex items-center gap-2">
          <Badge variant={accent === 'US' ? 'default' : 'outline'} className="cursor-pointer" onClick={() => setAccent('US')}>US</Badge>
          <Badge variant={accent === 'UK' ? 'default' : 'outline'} className="cursor-pointer" onClick={() => setAccent('UK')}>UK</Badge>
        </div>
      </div>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <Card className="md:col-span-2">
          <CardHeader>
            ...
            <CardDescription>Listen to the native pronunciation and record your own</CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
            ...
            <div className="flex flex-col items-center gap-4 py-8">
              ...
              <div className="flex gap-4">
                <Button size="lg" variant="secondary" onClick={playReference} disabled={loadingRef || isPlayingRef}>
                  {isPlayingRef ? <Volume2 className="mr-2 h-5 w-5 animate-pulse" /> : <Play className="mr-2 h-5 w-5" />}
                  Listen
                </Button>
                ...
                {userAudioBuffer && (
                  <Button size="lg" variant="outline" onClick={playUserRecording} disabled={isPlayingUser}>
            ...
            <div className="grid md:grid-cols-2 gap-8">
              <div className="space-y-2">
                ...
                <div className="bg-muted rounded-lg p-4 h-32 flex items-center justify-center relative overflow-hidden">
                  {loadingRef ? (
                    ...
                    <canvas ref={refCanvasRef} width={400} height={100} className="w-full h-full" />
                  )}
                </div>
              </div>
              ...
              <div className="space-y-2">
                <div className="flex justify-between text-sm font-medium">
                  ...
                  {userAudioBuffer && <span className="text-green-600">Good Match!</span>}
                </div>
                <div className="bg-muted rounded-lg p-4 h-32 flex items-center justify-center relative overflow-hidden">
                  {userAudioBuffer ? (
                    <canvas ref={userCanvasRef} width={400} height={100} className="w-full h-full" />
                  ) : (
